...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-SLDT16m") {
  const char source[] = "sldtw 0x11(%ebx,%esi,4)\n";
  uint8_t buffer[40] = {0};
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-STRm") {
  const char source[] = "str 0x11(%ebx,%esi,4)\n";
  uint8_t buffer[40] = {0};
  uint16_t *target = reinterpret_cast<uint16_t *>(&buffer[21]);
  *target = 0x0000;
  QBDI::rword targetAddr = (QBDI::rword)target;
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-VERRm") {
  const char source[] = "verr 0x11(%ebx,%esi,4)\n";
  uint8_t buffer[40] = {0};
  uint16_t *target = reinterpret_cast<uint16_t *>(&buffer[21]);
  *target = 0x0033;
  QBDI::rword targetAddr = (QBDI::rword)target;
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-VERWm") {
  const char source[] = "verw 0x11(%ebx,%esi,4)\n";
  uint8_t buffer[40] = {0};
  uint16_t *target = reinterpret_cast<uint16_t *>(&buffer[21]);
  *target = 0x0033;
  QBDI::rword targetAddr = (QBDI::rword)target;
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-ARPL16mr") {
  const char source[] = "arpl %ax, 0x11(%ebx,%esi,4)\n";
  uint8_t buffer[40] = {0};
  uint16_t *target = reinterpret_cast<uint16_t *>(&buffer[21]);
  *target = 0x1002;
  QBDI::rword targetAddr = (QBDI::rword)target;
  ExpectedMemoryAccesses expectedPre = {{
      {targetAddr, 0x1002, 2, QBDI::MEMORY_READ, QBDI::MEMORY_NO_FLAGS},
  }};
  ExpectedMemoryAccesses expectedPost = {{
      {targetAddr, 0x1002, 2, QBDI::MEMORY_READ, QBDI::MEMORY_NO_FLAGS},
      {targetAddr, 0x1003, 2, QBDI::MEMORY_WRITE, QBDI::MEMORY_NO_FLAGS},
  }};
  vm.recordMemoryAccess(QBDI::MEMORY_READ_WRITE);
  vm.addMnemonicCB("ARPL16mr", QBDI::PREINST, checkAccess, &expectedPre);
  vm.addMnemonicCB("ARPL16mr", QBDI::POSTINST, checkAccess, &expectedPost);
  QBDI::GPRState *state = vm.getGPRState();
  state->ebx = (QBDI::rword)&buffer[0];
  state->esi = 1;
  state->eax = 0x0003;
  ...
  CHECK(ran);
  CHECK(*target == 0x1003);
  CHECK((vm.getGPRState()->eflags & (1 << 6)) != 0);
  for (auto &e : expectedPre.accesses)
    CHECK(e.see);
  for (auto &e : expectedPost.accesses)
    CHECK(e.see);
