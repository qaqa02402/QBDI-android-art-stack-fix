...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-ADC32rm") {
  const char source[] = "adcl 0x11(%ebx,%esi,4), %eax\n";
  uint8_t buffer[40] = {0};
  ...
  *target = 0x05050505;
  ...
  CHECK((finalState->eax & 0xffffffff) == 0x15151515);
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-ADD32rm") {
  const char source[] = "addl 0x11(%ebx,%esi,4), %eax\n";
  uint8_t buffer[40] = {0};
  uint32_t *target = reinterpret_cast<uint32_t *>(&buffer[21]);
  *target = 0x05050505;
  QBDI::rword targetAddr = (QBDI::rword)target;
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-AND32rm") {
  const char source[] = "andl 0x11(%ebx,%esi,4), %eax\n";
  uint8_t buffer[40] = {0};
  uint32_t *target = reinterpret_cast<uint32_t *>(&buffer[21]);
  *target = 0x0f0f0f0f;
  QBDI::rword targetAddr = (QBDI::rword)target;
  ...
  state->ebx = (QBDI::rword)&buffer[0];
  state->esi = 1;
  state->eax = 0xf3f3f3f3;
  vm.setGPRState(state);
  QBDI::rword retval;
  bool ran = runOnASM(&retval, source);
  CHECK(ran);
  QBDI::GPRState *finalState = vm.getGPRState();
  CHECK((finalState->eax & 0xffffffff) == 0x03030303);
  for (auto &e : expectedPre.accesses)
    CHECK(e.see);
  for (auto &e : expectedPost.accesses)
    CHECK(e.see);
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-OR32rm") {
  const char source[] = "orl 0x11(%ebx,%esi,4), %eax\n";
  uint8_t buffer[40] = {0};
  uint32_t *target = reinterpret_cast<uint32_t *>(&buffer[21]);
  *target = 0x05050505;
  QBDI::rword targetAddr = (QBDI::rword)target;
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-SBB32rm") {
  const char source[] = "sbbl 0x11(%ebx,%esi,4), %eax\n";
  uint8_t buffer[40] = {0};
  uint32_t *target = reinterpret_cast<uint32_t *>(&buffer[21]);
  *target = 0x05050505;
  QBDI::rword targetAddr = (QBDI::rword)target;
  ...
  CHECK(ran);
  QBDI::GPRState *finalState = vm.getGPRState();
  CHECK((finalState->eax & 0xffffffff) == 0x2b2b2b2b);
  for (auto &e : expectedPre.accesses)
    CHECK(e.see);
  for (auto &e : expectedPost.accesses)
    CHECK(e.see);
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-SUB32rm") {
  const char source[] = "subl 0x11(%ebx,%esi,4), %eax\n";
  uint8_t buffer[40] = {0};
  uint32_t *target = reinterpret_cast<uint32_t *>(&buffer[21]);
  *target = 0x05050505;
  QBDI::rword targetAddr = (QBDI::rword)target;
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-XOR32rm") {
  const char source[] = "xorl 0x11(%ebx,%esi,4), %eax\n";
  uint8_t buffer[40] = {0};
  uint32_t *target = reinterpret_cast<uint32_t *>(&buffer[21]);
  *target = 0x0f0f0f0f;
  QBDI::rword targetAddr = (QBDI::rword)target;
  ...
  state->ebx = (QBDI::rword)&buffer[0];
  state->esi = 1;
  state->eax = 0x33333333;
  vm.setGPRState(state);
  QBDI::rword retval;
  bool ran = runOnASM(&retval, source);
  CHECK(ran);
  QBDI::GPRState *finalState = vm.getGPRState();
  CHECK((finalState->eax & 0xffffffff) == 0x3c3c3c3c);
  for (auto &e : expectedPre.accesses)
    CHECK(e.see);
  for (auto &e : expectedPost.accesses)
    CHECK(e.see);
