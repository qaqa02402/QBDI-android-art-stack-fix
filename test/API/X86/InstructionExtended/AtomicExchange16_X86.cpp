...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-CMPXCHG16rm") {
  const char source[] = "cmpxchgw %bx, 0x11(%edx,%esi,4)\n";
  uint8_t buffer[40] = {0};
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-XADD16rm") {
  const char source[] = "xaddw %bx, 0x11(%edx,%esi,4)\n";
  uint8_t buffer[40] = {0};
  uint16_t *target = reinterpret_cast<uint16_t *>(&buffer[21]);
  *target = 0x1010;
  QBDI::rword targetAddr = (QBDI::rword)target;
  ...
  CHECK((finalState->ebx & 0xffff) == 0x1010);
  for (auto &e : expectedPre.accesses)
    CHECK(e.see);
  for (auto &e : expectedPost.accesses)
    CHECK(e.see);
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-XCHG16rm") {
  const char source[] = "xchgw %bx, 0x11(%edx,%esi,4)\n";
  uint8_t buffer[40] = {0};
  uint16_t *target = reinterpret_cast<uint16_t *>(&buffer[21]);
  *target = 0x1010;
  QBDI::rword targetAddr = (QBDI::rword)target;
